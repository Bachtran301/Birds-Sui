...
class BirdX {
    ...
    async playEggMinigame(telegramauth) {
        ...
        try {
            const joinResponse = await axios.get("https://birdx-api.birds.dog/minigame/egg/join", { headers });
            let { turn } = joinResponse.data;
            this.log(`Starting egg cracking: có ${turn} lượt`, 'info');

            const turnResponse = await axios.get("https://birdx-api.birds.dog/minigame/egg/turn", { headers });
            ...
            while (turn > 0) {
                const playResponse = await axios.get("https://birdx-api.birds.dog/minigame/egg/play", { headers });
            ...
            const claimResponse = await axios.get("https://birdx-api.birds.dog/minigame/egg/claim", { headers });
    ...
    async nangcap(telegramauth, balance) {
        ...
        try {
            const infoResponse = await axios.get("https://birdx-api.birds.dog/minigame/incubate/info", { headers });
            ...
            if (incubationInfo.status === "processing") {
                if (currentTime > upgradeCompletionTime) {
                    const confirmResponse = await axios.post("https://birdx-api.birds.dog/minigame/incubate/confirm-upgraded", {}, { headers });
                    if (confirmResponse.data === true) {
                        this.log("Upgrade completed", 'success');
                        const updatedInfoResponse = await axios.get("https://birdx-api.birds.dog/minigame/incubate/info", { headers });
    ...
    async upgradeEgg(headers) {
        try {
            const upgradeResponse = await axios.get("https://birdx-api.birds.dog/minigame/incubate/upgrade", { headers });
